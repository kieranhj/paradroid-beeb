...
that the listing matches the original/CE releases byte for byte) and emits:
  ...
  src/data/sndchat.asm       - the three title-chatter records, for the
                               PARBRF overlay rather than bank 4; see the
                               chatter note below
...
three records are 33, so they are emitted into src/data/sndchat.asm for the
briefing's own overlay, and bank 4 carries ONE 11-byte scratch slot -
.sndFxChat, effect number 29 - which BrChatter copies its chosen blip into
before posting the request. The driver is untouched by any of it: it starts
effect 29 through the ordinary sndFxTab walk and neither knows nor cares
that the slot's contents change. docs/layer-11f-frontend.md section 4b.
...
FX_NAMES = {
    ...
    29: "briefing chatter blip A (triangle) - PARBRF, not the bank table",
    30: "briefing chatter blip B (sawtooth) - PARBRF",
    31: "briefing chatter blip C (pulse) - PARBRF",
}
...
def main():
    ...
    chat = [
        ...
        "\\ GENERATED by tools/export_sound.py - do not edit by hand.",
        "\\ ============================================================",
        "\\ Effects 29-31, the same 11-byte format as sndFxTab's records,",
        "\\ but assembled into PARBRF rather than bank 4: three of them is",
        "\\ 33 bytes and the bank had 15. BrChatter copies the blip it",
        "\\ picks into sndFxChat (bank 4, one slot) and posts effect",
        f"\\ {CHAT_SLOT}. Every instrument named here is in sndInstTab.",
        "\\ ============================================================",
        "",
        ".brChatTab",
    ]
    for n in CHAT_FX:
        e = effects[n - 1]
        s = e['sim']
        chat.append("  EQUB " + ",".join(f"&{b:02X}" for b in e['bytes'])
                    + f"  \\ {e['n']:2d}: {FX_NAMES[e['n']]}")
        if s:
    ...
    print(f"sounddata.asm: {SHIP_FX} of {NUM_FX} effects + the chatter "
          ...
          f"= {total} B of data")
    print(f"sndchat.asm:   {len(CHAT_FX)} chatter records x 11 = "
          f"{len(CHAT_FX) * 11} B, in PARBRF")
