...
def expand(nibble):
    """The renderer's fontExpand table: 4 pixel bits -> one MODE 1 byte.

    A byte holding only logical 0 and logical 3 is n<<4 | n, because pixel
    p takes bit 7-p as its high colour bit and bit 3-p as its low one and
    colour 3 sets both.  That is the whole reason the font packs."""
    return (nibble << 4) | nibble


def check_roundtrip(rows8):
    """The correctness argument, checked at generation time.

    The port used to ship each cell as 16 MODE 1 bytes.  It ships the C64's
    own 8 raw bytes now and FontCell expands them, so what has to hold is
    that expanding a packed byte's two nibbles reproduces the MODE 1 pair
    exactly - byte i of the cell from the high nibble, byte i+8 from the
    low one.  If this ever fails the font is no longer two-colour and the
    packing is lossy, so it is an assert and not a warning."""
    mode1 = cell_bytes(rows8)
    for i, row in enumerate(rows8):
        assert mode1[i] == expand(row >> 4), \
            'left half of scanline %d does not round-trip (%02X)' % (i, row)
        assert mode1[i + 8] == expand(row & 0x0F), \
            'right half of scanline %d does not round-trip (%02X)' % (i, row)


def cell_bytes(rows8):
    """One MODE 1 cell: 8 scanlines of 8 pixels -> 16 bytes, left half then right.

    Kept as the reference the packed output is checked against - see
    check_roundtrip.  Nothing is emitted from it any more."""
    left, right = [], []
    for row in rows8:
        lo = hi = 0
        for n in range(4):
...
def main():
    ...
    out.append('\\ ============================================================')
    out.append('\\ The C64 text font at $7000, VERBATIM - 1 bit per pixel.')
    out.append('\\')
    out.append('\\ NOT converted to MODE 1. It used to be, at 32 bytes a glyph, and')
    out.append('\\ half of every one of those bytes was redundant: the artwork is two')
    out.append('\\ colour, logical 0 and logical 3, so each MODE 1 byte came out as')
    out.append('\\ n<<4 | n and carried four pixels in eight bits. Pairing a cell\'s')
    out.append('\\ byte i with its byte i+8 - the same scanline\'s left and right')
    out.append('\\ halves - gives back (left<<4) | right, which IS the C64 byte. So')
    out.append('\\ the packed form is not an encoding this port invented; it is the')
    out.append('\\ original data, and FontCell in main RAM expands it as it draws.')
    out.append('\\ export_font.py asserts the round-trip for every cell it emits.')
    out.append('\\')
    out.append('\\ IT IS 8 x 16, NOT 8 x 8: the glyph top half is at C64 code c and')
    out.append('\\ its bottom half at c + $80. A cell is 8 x 8, which is 8 PACKED')
    out.append('\\ bytes, so a glyph is TWO STACKED CELLS, 16 bytes - top then bottom.')
    out.append('\\ That is why the four-row panel holds ONE line of text with a border')
    out.append('\\ row above it and another below.')
    out.append('\\')
    ...
    out.append('GEN_FONT_BYTES  = %d' % (len(GLYPHS) * 16))
    out.append('GEN_FRAME_CELLS = %d' % len(FRAME))
    out.append('GEN_FRAME_BYTES = %d' % (len(FRAME) * 8))
    out.append('')
    out.append('.textfont')

    for idx, (name, code) in enumerate(GLYPHS):
        rows = glyph_rows(mem, code)
        check_roundtrip(rows[:8])
        check_roundtrip(rows[8:])
        assert len(rows) == 16
        out.append('  \\ %2d  %s' % (idx, name))
        for half in (0, 8):
            out.append('  EQUB ' + ', '.join('&%02X' % b for b in rows[half:half + 8]))
    ...
    out.append('\\ ------------------------------------------------------------')
    out.append('\\ HALF glyphs, 8 packed bytes each, not 16: the box is 32 scanlines')
    out.append('\\ and')
    ...
    for idx, (name, code, half) in enumerate(FRAME):
        rows = glyph_rows(mem, code)
        data = rows[:8] if half == 'upper' else rows[8:]
