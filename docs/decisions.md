...
|---|---|---|
| Target machine | BBC B / B+ with **4 × 16K sideways RAM banks**, numbered 4–7 as the Master does (superseded by Layer 13b: `PARSWR` probes all sixteen and takes the highest four, so 4–7 is only the bare-`*RUN PARA` fallback) | 2026-08-16 |
...
| The title is a disc overlay, and a game over reaches it | `PARTITL` at `&3000`, loaded by `TitleSeq` at boot and after a game over ([DECISION 6] restored); `GoTitle` tears the IRQ down, restores the MOS's VIA state and the DFS workspace snapshot, and rebuilds. Layer 13d (superseded by the no-load branch: the driver is a bank-7 image copied to `&0900` since steps 3–4, the snapshot went with the last post-boot load in step 5, and `GoTitle` stopped tearing the rupture down in §20 — [`no-load.md`](no-load.md)) | 2026-08-20 |
...
| The deck maps ship ZX0-compressed | Decoded offline, byte-identical maps; the C64's RLE and both its decoders are gone, and bank 4 got ~1.1 K back. sideview stays in bank 7 — the approved move to bank 5 was unbuildable, `dfsSave` moved to bank 6 instead (`dfsSave` deleted 2026-09-09, no-load step 5) | 2026-08-20 |
...
| **The RAM recovery pass** | Five commits, 2026-08-25: dead tables deleted, the effect blitter to bank 5, the boot loop to `PARDEPK` (since folded into the resident depacker), one droid-icon copy in main RAM, `PAGEBANK`/`PNMIRROR` as subroutines. Main RAM 2 B → 639 B, every other region up too, for ~57 cycles a pass. [`ram-pass.md`](ram-pass.md) has the decisions, the rejections and the reserves | 2026-08-25 |
