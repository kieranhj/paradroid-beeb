...
\
\ ON A SPLIT PASS THE REPAINT BELONGS TO WINDOW B (2026-09-01), and
\ the caller is DoorAnimPaint's tail (door.asm) in both windows: in
\ window A on a whole pass, in window B on a split one, between
\ tranche B's restore and its draw. SprScanCls forces any sprite
\ under one of these tiles into tranche B, which is what makes the
\ deferred write safe.
.AnimPaint
  LDA animCount
  BEQ anp_x
