...
VERSION_LINE = "Release Candidate #6 (no load)"
...
\ ...and a SEVENTH, hoisted for the same reason on 2026-09-09, when
\ briefman.asm went bank-resident and moved from the very last block to
\ bank 5's. It was `BR_PO_ROW0 = DB_IMG_ROW`, and DB_IMG_ROW is bank
\ 7's, which is now later. condb.asm ASSERTs the two agree, so the
\ portrait cannot be moved on one side only. §13a called this exact
\ dependency out: it is what defeated the first attempt at that move.
BR_PO_ROW0  = 3                 \ = DB_IMG_ROW = DB_LINE_ROW0 + 2
ROW_BYTES  = 640
VIA_PORTB  = &FE40
...
\ ---- BR_BUF: where a briefing page depacks (no-load step 5) --
\ The briefing's arena is &4348-&4A00, measured, not assumed: &A5 was
\ seeded over &4220-&460F at the title, the briefing run, and the
\ memory read back -- 296 bytes written, all of them &4220-&4347, and
\ &4348 up untouched (docs/no-load.md 14b). THOSE 296 ARE NOT
\ EXPLAINED, so this does not spend to the line: &4500 is 440 clear of
\ the high-water mark and still holds the largest page.
\ IT IS ALSO WHAT THE STREAMS WERE ASSEMBLED AGAINST. A page's ZX0
\ stream is its row lists AND NOTHING ELSE, holding absolute addresses
\ that are correct only because tools/make_briefing.py laid them out
\ from BR_RECS -- so the two must agree, and BR_BUF_ASSUMED is the
\ tool's copy of this address. Nothing is relocated at run time.
\ THE TWO POINTER TABLES ARE BUILT BY THE DRIVER, not shipped: it walks
\ the depacked page for its $FF row terminators and fills brRowLo/Hi in
\ front of it. Carrying them inside the streams instead was measured at
\ 2,798 packed against 2,466, so the scan is worth 332 bytes of bank for
\ ~40 of code -- KC, 2026-09-09, docs/no-load.md 15d.
BR_BUF   = &4500                \ brRowLo, then brRowHi: 2 x BR_ROWS
BR_RECS  = BR_BUF + 2 * BR_ROWS \ ...and the depacked row lists
ASSERT BR_BUF == BR_BUF_ASSUMED
ASSERT BR_RECS + BR_PAGE_MAX <= PANEL_ADDR
...
\ bank for the briefing, which meant evicting the blitter and reloading
\ it on both exits; now the pieces simply live here and nothing loads.
\ BANK 5 IS THE BRIEFING'S RESTING BANK, which is what these three need
\ and the page streams do not: briefman is called per painted row
\ (BmBand), brExtra is read per CELL, and sndchat is briefman's. The
\ streams are read once each, by a depack that pages their bank itself,
\ so they go wherever they fit.
INCLUDE "src/data/sndchat.asm" \ the chatter's three records: 15 B, and
                               \ bank 4 had none -- see BmChatter
INCLUDE "src/briefman.asm"
INCLUDE "src/data/brextra.asm"
\ ...and keyredef's stream. The screen is assembled at BR_RECS in its
\ own block below and packed by the same pass that packs the two
\ overlays; it depacks OVER the briefing page, which BrPagePaint puts
\ back. 1,014 bytes of code for 776 of bank.
INCLUDE "src/data/krimg.asm"

\ ---- two of the briefing's five page streams (no-load step 5) ----
\ ZX0, one stream a page, generated by tools/make_briefing.py and
\ checked against beebasm's own PARMAN bytes by
\ tools/verify_brstreams.py. Each depacks WHOLE into BR_RECS, so no
\ stream may span a bank. They are pure data and nothing in this bank
\ reads them -- the depack is driven from main RAM with the bank paged.
\ WHICH BANK EACH ONE IS IN IS NOT ARBITRARY: keyredef's 776 needs a
\ hole only this bank has, and what is left over then decides the rest.
\ docs/no-load.md 17 has the packing and what it leaves in each bank.
\ AT THE TAIL FOR THE ORDINARY REASON: nothing in this bank ALIGNs,
\ so position costs nothing, and the tail is where the gauge reads them.
INCLUDE "src/data/brstream2.asm"
INCLUDE "src/data/brstream3.asm"
.spr_end
SAVE "PARASPR", spr_start, spr_end, DATA_LOAD, DATA_LOAD

\ ============================================================
\ The redefine screen — assembled where it is DEPACKED, not run
\ ============================================================
\ LAYER 11f's CTRL+R screen. It was inside PARMAN, in bank 5, and is
\ now a ZX0 stream in that bank instead (krImg, included up there):
\ 1,014 bytes of code for 776, which is what the packing needed.
\ IT IS ASSEMBLED AT BR_RECS BECAUSE THAT IS WHERE IT RUNS. The
\ briefing arena holds one page's row lists at a time, and the page is
\ dead while this screen is up — BrKeyRedef depacks over it and calls
\ BrPagePaint afterwards, which depacks the page back. Nothing here is
\ position-independent, so it has to be built at its landing address:
\ the same arrangement PARBRF and the low overlay have, and the same
\ pack pass carries it (tools/pack_overlays.py, XKR).
\ IT RUNS FROM MAIN RAM NOW, which changes nothing it depends on. It
\ calls PARBRF (BrChar, BrClearRow, BrWaitField) and main RAM
\ (SetCRTCStart, keydown), and reads brExtra through BrChar with
\ bank 5 paged, exactly as it did from inside that bank.
CLEAR BR_RECS, PANEL_ADDR
ORG BR_RECS
GUARD PANEL_ADDR
.kr_start
INCLUDE "src/keyredef.asm"
.kr_end
SAVE "XKR", kr_start, kr_end, 0, 0      \ the pack pass; see XBRF
...
\ ZX0 stream. The X-name never reaches the disc: make_disc.py drops
\ it by name, in BUILD_ONLY. See src/data/brfimg.asm.
...
\ THERE IS NO SIZE ASSERT and that was tried: a stale assert makes a
\ CHANGED overlay unbuildable, because the assembly that would produce
\ the new bytes is the one it stops. build.ps1's loop compares the
\ whole stream against the block's bytes on every build instead, which
\ is the stronger check anyway. See tools/pack_overlays.py.
INCLUDE "src/data/brfimg.asm"
INCLUDE "src/data/lowimg.asm"

\ ---- and briefing pages 1 and 5 (no-load step 5) ------------
\ Here rather than in bank 5 because bank 5 is being kept for
\ keyredef's 776 and the briefing's resident code. This bank could
...
\ The briefing's SECOND page stream, the largest of the five and the
\ one no other bank had room for -- docs/no-load.md 17.
\ BEHIND plandata.asm's ALIGN with xfericon.asm and the title artwork,
\ so it costs this bank its own size and nothing more -- in front of
\ the ALIGN the padding rolls a page and the bank pays 256 more.
INCLUDE "src/data/brstream1.asm"
...
\ ============================================================
\ The old record layout — THE ORACLE, and nothing else
\ ============================================================
\ src/data/briefing.asm is what PARMAN used to carry: the row lists in
\ the layout the bank-resident renderer walked. The game does not read
\ it any more — it reads the ZX0 page streams — but it is still
\ generated and still assembled, HERE, purely so that
\ tools/verify_brstreams.py has beebasm's own bytes to diff each
\ stream against. That check is what proves the Python blob builder
\ lays a page out exactly as the assembler does, and it is worth a
\ block that costs nothing on the disc: XREC is in make_disc.py's
\ BUILD_ONLY set and is dropped there.
CLEAR SWRAM_BASE, SWRAM_BASE + &4000
ORG SWRAM_BASE
.rec_start
INCLUDE "src/data/briefing.asm"
.rec_end
SAVE "XREC", rec_start, rec_end, 0, 0
