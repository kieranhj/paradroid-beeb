...
\ ---- three of the briefing's five page streams (no-load step 5) ----
...
\ WHICH BANK EACH ONE IS IN IS NOT ARBITRARY. keyredef packed is 776
\ and this is the only bank with a hole that big, so it is kept one:
\ these three take 1,744 of 2,650 and the 906 left is keyredef's.
\ Pages 1 and 5 are in banks 6 and 7 for the same kind of reason --
\ docs/no-load.md 15e has the packing and what it leaves over.
...
\ keyredef's 776, and this bank's 442 is the only hole page 1's 409
\ leaves usable at all. See the note in the PARASPR block above.
...
\ The briefing's fifth page stream. This bank's remaining 462 is
\ briefman's (456) in stage 3, which is what decided page 5 goes here
\ and page 1 does not -- docs/no-load.md 15e.
