...
wanted, so no two runs were comparable. Today's tree is **22,549 instructions** (measured
2026-09-11; it was 23,528 when the tool was checked in). It is blind to
...
- `VDU 22` makes the OS clear `&3000–&7FFF` (what it still thinks is its screen). Data loaded
  ...
  **The mode change is the LAST thing boot does before the title (2026-08-31)**: every bank load
  ...
  was written for; it is a bank-7 copy since no-load step 4, and since step 5 `PARBRF` is not a
  disc file either. The blank still covers `PARAFNT`'s and `PARXFER`'s loads in `.start`, which
  come after `SetupMode`.)
...
exit any more. It cost the code image its last bytes — **`code_end` was then exactly `&3000`** (43 B free as of 2026-09-11) —
...
`droid.asm`, `sound.asm`, `consolesel.asm` and, on debug builds, `dbgkill.asm` and `dbgdeck.asm`
(`zx0depack.asm` was one of them until the depacker went resident in the code image) are included from inside the `PARADAT` block, next
...
- `paradroid_ce.lst` — raw C64 disassembly in the project root (committed since 2026-09-09, KC on issue #3). The
...
- Debug builds are switched by constants at the top of `main.asm` — `DEBUG_RASTER`, `DEBUG_DRAW`,
  ...
  `DEBUG_INVULN`, `DEBUG_DECK`, `DEBUG_KILL`, `DEBUG_REDRAW`, `DEBUG_TRCHK` (a proof, not a
  readout: counts tranche-split pairs that break the split's rule; off even in dev builds). Each carries a header explaining what it shows and
