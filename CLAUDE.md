...
| Bank 7 | **131 B** — measured 2026-09-09, after BUGS.md #24’s `xfpause.asm` (35) and §20’s five bytes of title overlay; it was **171**, **and 33 B of `plandata.asm`'s `ALIGN` pad on top of it**: the pad was 208 and no-load step 6 put briefing page 5's first chunk (175) in it for nothing, exactly as `consolesel.asm` does in bank 4. it was 759 before the briefing's `brstream1` (604 packed) went in. The `no-load` branch's packing pass took it from 25 B to 3,021 (transfer board, lift screen, portrait pool, `poLut`; `docs/no-load.md` §4); step 3 SPENT 1,840 on the title artwork and the high-score screen, and step 4 a further 422 on the title overlay's image and `TiResident`. Before the branch it was 25 B, measured 2026-09-02 by bisecting a `SKIP` at `.LvStart7` |
