...
- `ref/paradroid_ce.lst` — raw C64 disassembly (gitignored; supply locally). The `_ce` suffix
  predates this project; the listing is original/CE lineage, so it is not misleading.
- `ref/paradroid_ce_annotated.asm` — annotated disassembly, generated by `annotate.py`
