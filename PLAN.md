...
| **The C64 loading intro** — planned 2026-08-26 | The three-robots screen and its lightning flash, ripped from a VICE dump ([`docs/graphics.md`](docs/graphics.md) §10) and reproduced as `PINTRO`, a separate exe `!BOOT` runs first **on `-Intro` builds only** (default builds boot straight into the game): static MODE 1 image, the whole effect as palette animation, one mid-screen palette split (budgeted by scarybeasts), any key exits into the game load. Plan, decisions and the as-built record in [`docs/intro.md`](docs/intro.md). scarybeasts' 15 kHz MOD player joins it later — animation budget revisited against its measured spare cycles, and the two effect calls move onto its cycle-counted schedule | **v1 built 2026-08-26** — verified in jsbeeb end to end; awaiting the MOD player |
