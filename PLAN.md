...
| Transfer: the two droid info screens | `ShowXferInfo` (`$3734`) shows two full-screen robot data pages before the board; the port goes straight to side select. [DECISION 8] in [`docs/layer-10-transfer.md`](docs/layer-10-transfer.md). **Confirmed still wanted, 2026-08-21** — queued behind `PrintTokenString` | not started |
...
Also unfinished, but small and not features: the console database page's portrait has never had an
**in-game play-check** (it was verified from the buffer).
...
| | |
|---|---|
| **RAM** | Measured from the build of 2026-08-21, not remembered: main RAM **5 B below `&3000`** (`code_end` = `&2FFB`) plus ~50 B in pieces elsewhere, and bank 4 **60 B** — up from 3 because the sixteen-row change collapsed three copies of the `t1i3` restore into one in `ReframeView` ([`docs/layer-9-hud.md`](docs/layer-9-hud.md) §6g). The sound driver, its data and every trigger had taken the lot, even after the char bitmaps and remap were ZX0-packed and `charSlot` nibble-packed to pay for it — [`docs/layer-11e-sound.md`](docs/layer-11e-sound.md) §6. **The build PRINTs bank 4's fuel gauge every run**, and the others are `&C000` minus the end addresses it PRINTs beside it. Bank 5 **1,033 B**, bank 6 **53 B**, bank 7 **314 B**, all paged out during play and unreachable from the main loop. **Anything new needs something moved first**; [`docs/memory-map.md`](docs/memory-map.md) lists the reservoirs left |
...
| Play area | **320 × 120** — 10 tiles wide, 15 character rows. The 16th row went to the single hardware wrap, which is what smooth vertical scrolling costs. **KC closed this 2026-08-21: 120 is what we ship** — the 20K wrap is not being pursued | 2026-08-05, closed 2026-08-21 |
...
| **The picture sits 4 rows lower on the tube** | `FRAME_DROP_ROWS` moves VSync four rows earlier (`TAIL_R7` 8 → 4) and `T1_I1` the same four rows later, so the panel and play area drop 32 scanlines without the frame or the cycles changing. It forced `R7 = 255` out of fire 1 and into `RuptVSync`: at `TAIL_R7` = 4 the 7-row panel cycle reaches the stale value and fires a VSync of its own, which blanked the play area entirely. **The title did not move** — it runs under `SetupMode`'s plain frame. [`docs/layer-3-scroll.md`](docs/layer-3-scroll.md) | 2026-08-21 |
| **Every non-gameplay screen shows all 16 rows** | The console and its three pages, the four information screens and the game over's wash join the transfer board, the lift's deck select and the deck plan. Set on entry, restored in **one** place — `ReframeView` — which freed 47 B of bank 4. The **ported** pages (database, information screens, game over) moved down one row onto the C64's own rows; the console main screen stays plotted from row 0 per KC's earlier rule. [`docs/layer-9-hud.md`](docs/layer-9-hud.md) §6g | 2026-08-21 |
...
| 14 | **The visual pass, deliberately last.** Every deck's and every screen's palette settled in one sitting with `tools/palette_lab.py`, plus redrawing the characters that fight MODE 1. Carries two referred items: the ALERT lamp's blinking fourth state, and the cleared-deck repaint, whose approximate greys are a palette choice like any other | **TODO** [`docs/layer-14-visual.md`](docs/layer-14-visual.md) |
