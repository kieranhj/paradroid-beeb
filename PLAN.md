...
| Droids inside walls | `BUGS.md` #8, reported from play 2026-08-15. **A correctness fault, not polish** — a droid on a solid cell can never leave one. Not reproduced idle; spawn ruled out arithmetically; suspects are a droid caught in a closing doorway and `DrReverse` leaving one facing a wall. The next step is a 30-byte detector, not more reasoning |
...
| Raster-ordered sprite updating | Flicker, and probably `BUGS.md` #3 with it. The only sprite-pool work still open. **Now measured, 2026-08-15**: the restore always runs in the off-display window and the draw is still running with the beam over it on **120 passes of 128**. The cheap fix is ruled out — the window is 184 scanlines, 11,776 cycles, against 24,000 for the draw — so it needs the pool ordered against the beam, which needs the sprite-overlap problem solved first. See [`docs/layer-5-blitter.md`](docs/layer-5-blitter.md) |
