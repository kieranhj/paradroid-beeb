...
**Layers 0–11, 13a/13d and 15 are built; 12, 13b/c and 14 are the roadmap.** The port boots to a playable game: the
...
**And since Layer 15 (2026-08-25) it has an endgame and a progression**, which it had never
had: `shipNumDroids` was maintained and never read. Clearing a deck pays 500, sounds `$17`
and takes the cleared floor; clearing the last deck on a ship pays 2,000 and puts up
`ShowShipClear` before `_entership` boards the next one — a fresh roster, the next name, the
alert down and the captured droid carried across at seven energy. **The game never ends**:
`shipLevel` caps at 8 as `$15F5` does and the eight names cycle under a difficulty that has
stopped rising, so it is survived rather than won.
...
| | | |
|---|---|---|
| ~~**The deck goes dark when it is cleared**~~ | **DONE.** Landed as layer-14 DECISION 6 (2026-08-24) and completed by Layer 15 (2026-08-25). KC's palette-only repaint, as ruled on 2026-08-21: the floor takes blue when nothing but the player is left, blue standing in for the dark grey of scheme 7 that MODE 1 has not. Layer 15 then added the rest of `$17DC` — the 500 points, the `$17` chord and the ship-clear test — and fixed the re-fire: walking back onto a cleared deck re-ran the whole event, points included, because the port had no equivalent of `$1388`'s test. `docs/layer-14-visual.md` DECISION 6, `docs/layer-15-endgame.md` DECISION 6 | ✅ done |
...
| | |
|---|---|
| **RAM** | **Measured from the build of 2026-08-24**, not remembered. Every region is single or low double digits: main RAM **2 B** below `&3000` (`code_end` = `&2FFE`, and it went UP: layer-15 DECISION 1 moved `DEBUG_DECK`'s 69-byte arm out to `src/dbgdeck.asm` in bank 4), bank 4 **3 B** after Layer 15's endgame spent the space pass's 105 on 2026-08-24 and DECISION 6's cleared-deck fix took the rest on 2026-08-25 — 112 B of per-deck tables that nothing read, less 15 spent on a self-healing page pad in `sound.asm`, [`docs/memory-map.md`](docs/memory-map.md) §"Layer 15 space pass"; it was 8 before, and **17 B of `colourMap`'s original 162 B of `ALIGN` padding are still unspent** on top, which anything assembled before that `ALIGN` rides for nothing (layer-9 DECISION 18 — **deleting the `ALIGN` recovers nothing**, `tiledefs` just pads instead), `PARBRF` **3 B** (ceiling `&0800`, not `&0C90` — the MOS owns the page above), the low overlay **1 B**, `lowcode2` **8 B**, bank 6 **4 B**, bank 7 **314 B**. Bank 4 went to 26 B when Layer 14's floor dither paid for itself and spent it again on the text palettes ([`docs/layer-14-visual.md`](docs/layer-14-visual.md)); that padding is where `deckTextPal` and `consolesel.asm` both live. The roomy ones are bank 5 **1,033 B** and bank 7 **314 B**, both paged out during play and so unreachable from the main loop, plus `PARMAN`'s unused 11 K when the briefing has bank 5. Layer 11e's sound driver took bank 4's margin and Layer 11f's front end took what the sixteen-row change had bought back. **The build PRINTs bank 4's fuel gauge every run**, and the others are `&C000` minus the end addresses it PRINTs beside it. **Anything new needs something moved first**; [`docs/memory-map.md`](docs/memory-map.md) lists the reservoirs left |
...
| 14 | **The visual pass, deliberately last.** Every deck's and every screen's palette settled with `tools/palette_lab.py`, plus redrawing the characters that fight MODE 1. **STARTED 2026-08-22**: the deck floor is dithered to half intensity (DECISION 1), which turned out to buy a fifth tone that decks 0, 5 and 9 spend on grey (DECISION 2); dithering the static screens was tried and reverted, and they take a solid per-deck background instead (DECISION 4). **The cleared-deck floor landed 2026-08-24** (DECISION 6): blue when nothing but the player is left, which is the C64's scheme-7 swap with blue standing in for a dark grey MODE 1 has not. Still to do: the remaining screens' palettes, the characters that fight MODE 1, and the ALERT lamp's blinking fourth state | **in progress** [`docs/layer-14-visual.md`](docs/layer-14-visual.md) |
| 15 | **The endgame — clearing a ship, and the next one.** **BUILT AND VERIFIED 2026-08-24.** The port had **no win condition and no ship progression at all** — `shipNumDroids` was maintained and never read. All five tasks landed: 500 points and `$17` for a deck, a `shipClear` byte, 2,000 points for a ship, `ShowShipClear` as a fifth information screen (the C64's own sixteen-byte token list), and `GameStart` split at `$1242`/`$1289` so `EnterShip4` is callable alone — which is what stops `CombatInit` wiping the score on a ship transition. **The game never ends**: `shipLevel` caps at 8 as `$15F5` does, freezing the difficulty, while a separate `shipName` cycles the eight names under it (DECISION 5, revised by KC 2026-08-25 — the C64 pins the name along with the level, so cycling is the port's one improvement on it). Two pre-existing bugs fell out: the string exporter dropped the C64's last string ("awarded") and `hsArmed` was being written from the wrong bank. **2026-08-25** fixed two more from KC's playtesting: re-entering a cleared deck replayed the whole event and its 500 points (DECISION 6, the C64 skips `RunDroids` at `$1388`), and the ship reset was missing `$1293`'s alert and `$12A7`'s energy — both because `CombatInit` does them and it sits above the T5 split (DECISION 7, with `_entership` audited store by store). **Bank 4: 3 bytes free, main RAM 2.** [`docs/layer-15-endgame.md`](docs/layer-15-endgame.md) | a ship cleared by playing, not by poking |
