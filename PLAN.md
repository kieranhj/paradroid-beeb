...
| | |
|---|---|
| **RAM: Layer 13a done, 2026-08-19** | main RAM **66 B** (was 30), bank 4 15 B, bank 5 1,033 B, bank 6 **1,609 B** (was 40), bank 7 **4,410 B** (was 282), plus 202 B at `&3D36`. **+6,003 bytes**, no cycle cost and no behaviour change. **11c's loop, 11d and 11e are all unblocked**, and bank 6 has stopped being the tight bank. Bank 4's 15 B is now the only one left. The font ships 1bpp — the C64's own bytes — one `FontCell` in main RAM replaces four copies of the same loop, and the string table exists once instead of twice. See [`docs/layer-13-ram-pass.md`](docs/layer-13-ram-pass.md) |
