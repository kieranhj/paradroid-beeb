...
| | |
|---|---|
| **RAM: Layer 13a's tier 1 is done, 2026-08-19** | main RAM **66 B** (was 30), bank 4 15 B, bank 5 1,033 B, bank 6 **67 B** (was 40), bank 7 **2,874 B** (was 282), and **1,744 B free at `&3730`** where the font used to be. **+4,461 bytes**, no cycle cost and no behaviour change. **11c's loop and 11d are both unblocked.** The font now ships 1bpp — the C64's own bytes — and one `FontCell` in main RAM replaces four copies of the same loop. Next is TASK 7: `constrings` exists twice, 1,731 bytes each, and the freed region takes one copy. See [`docs/layer-13-ram-pass.md`](docs/layer-13-ram-pass.md) |
