...
| `DoHighScore` | `$E4E5` — **BUILT AND VERIFIED 2026-08-21/22** in the `PARTITL` overlay (Layer 11f F1), table in bank 7, entry drawn over the 999 page. And the thing that displays it exists too: **the briefing scroller (11f F4/F5) is BUILT** — the five C64 pages scroll under the rupture at the original's own speeds and dwells, driven from `PARBRF` at `&0400` with the text in an evicted bank 5, and page 5 shows the live table. Open in the layer: the page-5 portrait (a design question — the C64 floats it as a hardware sprite), the panel's HIGH/LOW box content, title chatter (F2, deferred), and the exit-load trim (F6). [`docs/layer-11f-frontend.md`](docs/layer-11f-frontend.md) | portrait + panel box |
