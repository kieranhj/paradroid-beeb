...
| | | |
|---|---|---|
| **Sound** | Layer 11e stages 0–3 **BUILT 2026-08-21 — the game PLAYS with sound**: driver in bank 4, every in-game trigger wired (fire, hits, kills, collisions, lift, console, transfer, wash, per-deck hum via `SndAmbient` + the `LoadDeck` patch), `GoTitle` silences at the seam. **What remains is stage 4, by ear with KC**: the sub-floor six → periodic noise, the noise scale factor, the transfer-verdict mapping, A/B against VICE — plus pause/volume keys and title chatter, both deferred features. [`docs/layer-11e-sound.md`](docs/layer-11e-sound.md) | stage 4 tuning |
