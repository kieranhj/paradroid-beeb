...
| Game over → the title | **DONE 2026-08-20.** `GoTick7` → `GoTitle`: `UninstallIrq`, `SetupMode`, `RestoreDfsWs`, `TitleSeq`, `GameStart`. The title is the `PARTITL` disc overlay at `&3000` again ([DECISION 6] restored) and boot shares `TitleSeq`. Verified in jsbeeb over two consecutive game overs. The hazard found on the way — the low overlay buries DFS's workspace AND the extended vector table, so any post-`PageLowIn` filing call crashes — is fixed by the `dfsSave` snapshot in bank 6; the story is in [`docs/layer-11-sound-title.md`](docs/layer-11-sound-title.md) §11c | done |
| The 48 × 84 droid portrait | **PORTED 2026-08-20** — the "6 K / 24 K" costing was wrong on both ends: the unique image pool is **4,032 B of the C64's own bytes** (63 sprites; the right column was always mirrored at runtime), and `portrait.asm` expands it at draw time like `FontCell` does the font. `PoDraw` in bank 7 reproduces `BuildIntroSprites` **byte-for-byte** — verified against a Python reference over types 0, 14 and 23, 0 mismatches — and the console database page draws it in place of the rotor-and-digits stand-in (`droidicon7.asm` deleted). Bank 7 has 826 B left. `NewShipInfo`, `ShowXferInfo` and the game-over 999 are now unblocked, but each needs `PoDraw`'s rectangle parameterised — it is anchored to the database page's — and the database page itself still wants an in-game play-check | done (database page) |
...
| | |
|---|---|
| **RAM** | main RAM **161 B in five pieces** — 110 below `&3000` (`code_end` = `&2F92`), 0 in `lowcode`, 36 in `lowcode2`, 7 in `lowbss`, 8 in the `PARAFNT` tail — and bank 4 **1,161 B** — the ZX0 deck maps gave back what the RLE was spending, see [`docs/layer-13d-space.md`](docs/layer-13d-space.md). Bank 5 1,033 B, bank 6 **63 B** (the 912 B `dfsSave` snapshot took the rest), bank 7 **826 B** (the portrait pool and renderer took their 4,864). The game-over → title seam (2026-08-20) spent ~213 B of main RAM on `TitleSeq`/`GoTitle`/`UninstallIrq`/the DFS snapshot helpers and took the title's 1,345 B OUT of bank 7 (`PARTITL` is a disc overlay again). The sound driver still has to be resident. [`docs/memory-map.md`](docs/memory-map.md) lists the reservoirs left |
