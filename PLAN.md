...
| | | |
|---|---|---|
| **Sound — all of it** | Layer 11e. **PLANNED 2026-08-21 with KC — [`docs/layer-11e-sound.md`](docs/layer-11e-sound.md)** is the spec: the full C64 effect inventory (31 effects, 2 sounding voices — SID voice 3 is muted and serves as an RNG), an IRQ-driven 50 Hz `SndTick` **in bank 4, paged in by the IRQ itself via `ROMSHAD`** (amending, not breaking, the bank rule), effect records converted offline by a new `tools/export_sound.py`. Bank 4's 1,161 B is the budget and the plan spends nearly all of it. **Stage 0 verifies the chip encoding in the emulator first** — it is still recalled, not measured | the biggest hole in the game as it plays |
