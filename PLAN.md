...
| 15 | **The endgame — clearing a ship, and the next one.** **SCOPED 2026-08-24, nothing built.** The port has **no win condition and no ship progression**: `shipNumDroids` is maintained and never read, `NewShipDroids` is `NextLevel`'s roster half only, and `GameStart` has `_entership` fused into it. Missing: the deck payout (500), the ship payout (2,000), both sounds, `ShowShipClear` ("congratulations! ship is now clear of all robot activity") and the `INC shipLevel` that starts ship 2. **The screen is the cheap part** — `infoscr.asm` already builds three of exactly its shape. **Bank 4 has ~25 B of headroom against ~50 B of estimate, so the space pass comes first** | **TODO** [`docs/layer-15-endgame.md`](docs/layer-15-endgame.md) |
