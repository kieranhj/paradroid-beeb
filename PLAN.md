...
| ~~Sprites are all one colour~~ | **Done 2026-08-19.** The C64 colours every sprite: enemy droids **black** (`$18FA`), the player white, the deck's `clr6_xfer` in transfer mode (`$3E21`), and a **flash below energy 8** (`$3DEE`). All four are built. The compiled blitter can be told a colour after all — the artwork is stored at logical 3, so choosing a colour is choosing a nibble, and eleven zero page bytes carry it at one cycle per opaque byte and no bank at all. This is what made Layer 9's [DECISION 5] wrong. See [`docs/layer-5-blitter.md`](docs/layer-5-blitter.md) § "Colour is not baked in" and the two numbered decisions in [`docs/layer-7-combat.md`](docs/layer-7-combat.md) |
...
| `&2FCA–&2FFF` | **54 B free** | **the binding constraint**, and where Layer 11e's sound driver has to go — the IRQ reads no sideways bank. It was 24, then 30, then 148 after Layer 13a's TASK 6 and TASK 8; the sprite colour work spent 94 of that on `SprSetColour`, the pattern table, the per-slot array and the player's colour arms |
