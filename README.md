...
|---|---|
| **Z** / **X** | left / right |
| **K** / **M** | up / down. On the lift screen they move along the shaft; on the intro manual **K** pauses the scroll and **M** doubles it |
| **L** | fire — it only fires. Inside the lift screen or the console it commits and leaves, as on the C64, and it starts the game from the title or the manual |
| **SPACE** | transfer — hold it and a transfer triggers on contact without needing a direction. On a lift platform or at a console it opens the lift screen or the console, and pressed again it leaves (or, on the lift screen, rides to the deck chosen). Also starts the game |
| **A–Z**, **DELETE**, **RETURN** | high-score entry: type the initials; DELETE steps back and RETURN finishes early |
...
Main RAM is the binding constraint throughout: the code image runs to within a few dozen
...
paradroid_ce.lst   The C64 disassembly - the specification, and the input to tools/
annotate.py        Generates the annotated C64 disassembly
build.ps1          The Windows build; Makefile is the POSIX one, and the two produce the same disc
...
**The C64 game remains the copyright of Andrew Braybrook and Hewson Consultants.** Two things
derived from it are committed. `paradroid_ce.lst`, in the project root, is a disassembly of the C64
binary and the port's specification. `src/data/` is the data the port needs, converted from that
listing into BeebASM form. The normal build uses only `src/data/`, and the extraction tools below
read the listing.
...
`make data` regenerates `src/data/` from the listing (it needs Pillow), and `make world` does that
and then builds. The inspection tools run by hand:
...
license — the C64 disassembly `paradroid_ce.lst`, the converted game data in `src/data/`, the
vendored loading intro in `pdloader/` — so no
